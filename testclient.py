...
scaled_crosshair = pygame.transform.scale(crosshair,(int(30*(SCREEN_WIDTH/1920)),int(30*(SCREEN_HEIGHT/1080))))
...
while game_is_running:
    ...
    player2 = Player(screen,player2interim.health,player2interim.ammo,player2interim.cooldown,player2interim.bullet_interim_list,player2interim.vertical_velocity,player2interim.is_in_air,player2interim.can_jump,player2interim.is_alive,player2interim.update_timer,player2interim.player_type,player2interim.players_action,player2interim.action_number,player2interim.flip,player2interim.x*(screen.get_width()/1920),player2interim.y*(screen.get_height()/1080))
    # player2.animate()
    ...
    if is_game_over(player1,player2) and (not bullet_group_player1) and (not bullet_group_player2):
        font = pygame.font.Font('introrustg-base2line.otf',100)
        if game_over_type ==1:
            game_over_text = font.render('YOU '+ ' WIN!!!!!!!!',True,(255,255,255))
            screen.blit(game_over_text,(int(((SCREEN_WIDTH/2)-200)*(SCREEN_WIDTH/1920)),int((SCREEN_HEIGHT/2) * (SCREEN_HEIGHT/1080))))
